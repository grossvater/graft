...
# $Id: graft.pl,v 2.12 2017/02/16 15:56:04 psamuel Exp $
#
...
sub checksum
{
    ...
    open FILE, $file or die "Failed to open $file for checksum calculation: $!\n";
...
sub init
{
    ...
    usage() unless (exists $option{delete} or exists $option{install} or exists $option{prune});

    # Options -d, -i and -p are mutually exclusive

    usage() if (
		    (exists $option{delete} and exists $option{install})
		    or
		    (exists $option{delete} and exists $option{prune})
		    or
		    (exists $option{install} and exists $option{prune})
		);
	...
	usage() if (exists $option{preservePermissions} and (exists $option{delete} or exists $option{prune}));

	# -P and -u are mutally exclusive

	usage() if (exists $option{preservePermissions} and exists $option{superuser});
    }
    ...
    {
	usage() if (exists $option{neverGraft} and (exists $option{delete} or exists $option{prune}));
    }
    else
    {
	...
	++$config{verbose} if (exists $option{verbose} or exists $option{veryVerbose});
    ...
    if ($config{superuser})
    {
	if (exists $option{preservePermissions})	# Toggle preserve permissions flag
	{
    ...
    $config{superuser} = 0 if (exists $option{superuser} and $config{superuser});
...
sub install
{
    ...
    %excludes = cat($config{graftExclude}) if (exists $sfiles{$config{graftExclude}});
    %includes = cat($config{graftInclude}) if (exists $sfiles{$config{graftInclude}});
	...
	if (-f "$source/$dir/$config{graftIgnore}")
	{
		...
		message(
			    tag	=> 'IGNORE',
			    msg	=> "exclude file $source/$dir/$config{graftExclude}, overridden"
		...
		message(
			    tag	=> 'IGNORE',
			    msg	=> "include file $source/$dir/$config{graftInclude}, overridden"
				   . " by bypass file $source/$dir/$config{graftIgnore}"
		    ) if $config{veryVerbose};
	    }
		...
		message(
			    tag	=> 'IGNORE',
			    msg	=> "config file $source/$dir/$config{graftConfig}, overridden"
				   . " by bypass file $source/$dir/$config{graftIgnore}"
		    ) if $config{veryVerbose};
	    }
	...
	if (exists $sfiles{$config{graftExclude}})
	{
		...
		message(
			    tag	=> 'EXCLUDE',
			    msg	=> "directoy $source/$dir - listed in"
				   . " $source/$config{graftExclude}",
		    ) if $config{veryVerbose};
...
sub mkcopy
{
	...
	unless (exists $option{noexec})
	{
		...
		my (undef, undef, $mode, undef, $uid, $gid, undef, undef, $atime, $mtime)
		    = stat "$source/$file";
		...
		utime $atime, $mtime, "$target/$file"
		    or die "Could not set access and modification times on $target/$file: $!\n";

		system("ls -l $target/$file");
	    }
	}
	...
	else
	{
		...
		unless (exists $option{noexec})
		{
			...
			or die "Failed to copy $source/$file to $target/${file}$config{configSuffix}: $!\n";
			...
			chmod $mode, "$target/${file}$config{configSuffix}"
			    or die "Could not create $target/${file}$config{configSuffix}: $!\n";

			chown $uid, $gid, "$target/${file}$config{configSuffix}"
			    or die "Could not set ownership on $target/${file}$config{configSuffix}: $!\n";

			utime $atime, $mtime, "$target/${file}$config{configSuffix}"
			    or die "Could not set access and modification times on $target/${file}$config{configSuffix}: $!\n";
		    }
		}
	    }
	}
    ...
    logger(
		tag	=> 'IC',
		log	=> [ "$target/$file", 'file exists but is not a file or a symlink' ],
	    ) unless (exists $option{noexec});
...
sub prune
{
	...
	unless ("$source/$file" eq $link)
	{
		...
		message(
			    tag	=> 'RENAME',
			    msg	=> "$target/$file to $target/${file}$config{prunedSuffix}",
			) if $config{veryVerbose};

		unless (exists $option{noexec})
		{
		    rename "$target/$file", "$target/${file}$config{prunedSuffix}"
	...
	if (-l "$target/$file")
	{
		...
		if (-e "$target/${file}$config{configSuffix}")
		{
			...
			unlink "$target/${file}$config{configSuffix}"
			    or message(
					    tag	=> 'ERROR',
					    msg	=> "Could not unlink $target/${file}$config{configSuffix}: $!",
					);
		    }
		}
	    }
		...
		else
		{
			...
			message(
				tag	=> 'NOP',
				msg	=> "Preserving $target/${file}$config{configSuffix}"
...
Usage:
  $config{progname} -i ${option_C}[-P|u] [-l log] [-n] [-v|V] [-s|-t target] package package ...
...
Usage:
  $config{progname} -i ${option_C}[-l log] [-n] [-v|V] [-s|-t target] package package ...
